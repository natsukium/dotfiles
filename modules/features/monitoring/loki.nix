{ ... }:
{
  flake.modules.nixos.loki =
    ...
    in
    {
      options.my.services.loki.enable = mkEnableOption "Loki log aggregation";
      ...
      config = mkIf cfg.enable {
        ...
        # Tailscale-only network, but firewall is still strict -- open Loki
        # ingress so Alloy clients on other hosts can push.
