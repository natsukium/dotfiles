...
let
  ...
  # JSON Lines. Each output row is an id and a display line separated by
  # a tab, so fzf can hide the id in field 1 while the preview still
  ...
  #
  # Neither idle nor geometry: `idle_seconds` counts from the last read
  # and the preview reads whatever it lands on, so that column would
  # measure the picker; every window opens at the same size.
  #
  # Padding rather than further tab-separated fields: fzf renders tabs
  # at a single `--tabstop`, so every column would round up to the
  # widest one's stop. Only the title carries wide characters, and it is
  # last, so padding by `length` holds.
  roster = ''
    roster() {
  ...
  #
  # Stacked rather than side by side: half a window cuts off both the
  # grid and the title. fzf's `<N(…)` compares the preview's own width
  # in the primary layout, not the terminal's.
      ...
      selection=$(
        printf '%s\n' "$list" \
          | fzf --ansi \
                ...
                --preview-window='right,50%,<80(up,50%)'
      ) || exit 0
