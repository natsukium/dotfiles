...
Two keymap entries work around problems that live outside felis. On a JIS
keyboard the yen key still types =¥= even though macOS is configured to emit
=\=, because winit drops the substituted character unless an IME preedit is
active; rebinding the key here avoids waiting for a winit release. Shift+Enter
is already sent as =CSI 13;2u= under the Kitty keyboard protocol, but Claude
Code enables that protocol only for terminals on a hardcoded list, so I send the
Option+Enter sequence it does recognize as a newline instead. Both entries go
away once upstream catches up.
