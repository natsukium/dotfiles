...
=RUSTUP_HOME= and =CARGO_HOME= that keep the caches on D:, git, rustup with the
nightly toolchain, the MSVC build tools, cargo-nextest, and the runner itself. A
task registered at startup copies the runner across to =D:\runner= and runs the
daemon as SYSTEM. Copying on every boot keeps the version the flake pins as the
