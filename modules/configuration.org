...
register through nixpkgs' =services.gitea-actions-runner=; macOS has no such
module, so I provide =services.forgejo-runner= for Darwin, driven by launchd and
shaped like the NixOS option (instances under
=services.forgejo-runner.instances.<name>=) so a
runner reads the same on either platform.

Three modules share this file. The Darwin mechanism is neutral and
upstreamable, so I publish it as the public =flake.darwinModules.forgejo-runner=.
The per-class =my.services.forgejo-runner= wrappers bake my personal
defaults (instance URL, labels, niks3 client) and go in the internal
=flake.modules.{darwin,nixos}= registry, which =_class=-checks imports — a
distinct surface from the public =darwinModules= output. A host enables a
wrapper and supplies its own registration token.
...
*** Neutral Darwin module

nix-darwin has no Actions runner, so this module supplies one and nothing
personal — only =services.forgejo-runner=, with the same instance schema as
nixpkgs' =services.gitea-actions-runner= (=token=/=tokenFile=, =labels=, freeform
=settings=). Each
enabled instance becomes its own launchd job that registers, then execs the
daemon. Registration re-runs automatically whenever the token or labels change:
the job records a hash of =token= plus =labels= in =.registration-state= and
re-registers when it drifts, so a label edit takes effect on the next
activation without manual cleanup. Forgejo registration tokens are reusable, so
re-registering with the same token is fine.

#+NAME: forgejo-runner-darwin-module
#+begin_src nix
darwinModule =
  {
    config,
...
=PATH=. The token comes from either =token= (a plaintext string) or =tokenFile=
(an environment file holding =TOKEN==), exactly one of the two — matching the
NixOS module so a host reads identically on either platform. =settings= is a
freeform attrset rendered to =config.yaml= and handed to the daemon with
=--config=. Host-executed jobs inherit only the daemon's environment, so every
...
The daemons share one dedicated unprivileged user, not root. Besides least
privilege there is a concrete reason: a root daemon is a trusted-user client of
the nix daemon, which sends niks3-action down its post-build-hook push path — a
path the multi-user daemon ignores, so nothing reaches the binary cache. An
untrusted user takes the store-scan push path that actually uploads. The =_=
prefix is the macOS convention for hidden accounts, and 530 sits in the band
nix-darwin uses for its own service accounts (e.g. =_prometheus-node-exporter=
at 534) while staying unused.
...
One launchd job per enabled instance. systemd reads =tokenFile= as an
=EnvironmentFile=; launchd has no equivalent, so a =tokenFile= instance sources
it in-script to put =TOKEN= in the environment, while an inline =token= rides on
=EnvironmentVariables=. Either way the script sees =$TOKEN=. =coreutils= is
prepended to =PATH= unconditionally so the script's own =sha256sum=/=cut= resolve
even if a host trims =hostPackages=. Two launchd quirks shape each:
...
=users.knownGroups=; without these entries the user declaration would be a
no-op on an existing system.
...
Each instance's state directory must belong to the runner user before its
daemon starts, or a directory left owned by root blocks the runner from reading
its own =.runner= credential. The chown runs after the user exists
(=activationScripts.users=); =KeepAlive= restarts a daemon cleanly if it bounces
once before the chown lands.
...
The two platforms drive different runner options — =services.forgejo-runner= on
Darwin, nixpkgs' =services.gitea-actions-runner= on NixOS — but the Darwin module
mirrors the NixOS instance schema, so the wrappers can share one option surface
and one instance builder. The shared
=my.services.forgejo-runner= options give =enable=, an =url= (defaulting to
my Forgejo instance), =labels= a host can override, and a =tokenFile= path;
=mkInstance= fills the =default= instance identically from those plus the host's
name. The token is host-issued and host-specific, so the host — not this
feature — owns the whole secret: it declares the sops entry (and, on Darwin, the
owner the launchd job reads it as) and hands its path to =tokenFile=. Keeping the secret
store out of the feature is why =tokenFile= is just a path. Only the genuine
per-platform deltas stay in each wrapper: the =labels= default (=mkDefault=, so
the shared option can override), =hostPackages=, and — on NixOS — the
=forgejo-runner= package and Docker for container jobs. Both push to my binary
cache, so each =hostPackages= adds =nix= and the niks3 client.
...
1. =perSystem.packages.neovim= makes =nix run .#neovim= work on every
   supported system.
2. =flake.homeManagerModules.neovim= lets a home-manager config opt in
   with =my.programs.neovim.enable=, which installs the wrapped Neovim
   plus =neovim-remote= and exports =EDITOR=nvim=.
...
2. =flake.homeManagerModules.emacs= adds =my.programs.emacs.enable= for
