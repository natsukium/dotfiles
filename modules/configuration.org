...
I split this into three modules. The nix-darwin mechanism is neutral, so I publish
it as the public =flake.darwinModules.forgejo-runner=. The
...
Alongside the client, a small =felis-switch= helper pipes =felis sessions
list= through fzf and hands the chosen id to =felis sessions switch=, with a
preview pane rendering each session's grid in colour (=capture --ansi=). It
fills the gap left by the sequential =switch_{next,previous}_session=
keybindings, which can only step through sessions one at a time. The =run=
keymap action binds it to =Ctrl+Shift+p=, launching the picker as an
interactive TUI over the live grid.

felis keeps URL detection out of its core on purpose: OSC 8 hyperlinks are
clickable (=Ctrl+Click=), but the plain-text URLs that dominate real output —
nothing emitted an OSC 8 link around them — are not. That heuristic lives in
an external tool instead, the same split kitty's hints kitten draws. A
=felis-hints= helper is felis's side of it: the =pipe= action ships the
visible grid (plain and soft-wrap-stitched daemon-side) to =thumbs=, which
overlays single-key labels on every match and opens the pick. Bound to
=mod-shift+o=. Open-only for now — an insert-at-the-prompt mode would need
felis to hand the transient command its originating session id.

A second hint, =felis-nix-log= on =mod-shift+l=, ports the kitty
=Ctrl+Shift+l= binding: it scans the visible grid for a =nix log <drv>= line
and pages that derivation's build log. Unlike the URL hint it uses grep + fzf
rather than thumbs, because thumbs cannot suppress its built-in path/URL
matchers and kitty's value here is matching *only* the =.drv=; a lone drv —
the usual failed-build case — opens with no picker at all.
