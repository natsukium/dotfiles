...
I organize modules by functional domain (e.g., Shell, Networking, Version Control) rather
...
=authKeyFile= points to a SOPS-managed secret containing a Tailscale auth key. Auth keys
allow unattended authentication, which automated deployments and GitOps tools like comin
depend on.
...
Fish is my primary interactive shell. I chose it for its out-of-the-box experience:
syntax highlighting, autosuggestions, and tab completions work without configuration
or plugins. Among shells with broad environment and software support, fish gives the
most convenient and extensible interactive experience for the least setup effort.
...
This configuration is a remnant from before I made fish the default login shell.
Back then bash was the login shell and =.bashrc= launched fish, so these settings
ran in every interactive session. I keep them because bash still serves as the
fallback.
...
I enable Nushell for its native handling of structured data, similar to
...
I have not added any custom configuration yet — the defaults are enough for
exploratory use, and committing to Nushell-specific workflows would be premature.
...
[[https://starship.rs/][Starship]] is a cross-shell prompt written in Rust; I use it in every shell configured
here so the prompt looks the same everywhere. I have used it since its predecessor
[[https://github.com/matchai/spacefish][spacefish]], and its easy TOML-based configuration is the main reason it has stayed.
...
displaying a whale emoji (🐋) as a visual reminder. I have not used this in
several years, and it may no longer be relevant.
...
SSH-based commit signing. I chose SSH keys over GPG back when the key was already
required for push authentication, which spared me a separate GPG keychain. Git's SSH
signing support (added in Git 2.34) provides the same integrity guarantees as GPG
signing with simpler key management. Pushes have since moved to HTTPS (see
[[*GitHub CLI][GitHub CLI]]), so signing is now the one job the SSH key still does.
...
[[https://git-scm.com/docs/scalar][Scalar]] optimizes git performance for large repositories. I enable it specifically for
the nixpkgs repository, which contains over 600,000 commits and benefits from Scalar's
background maintenance (prefetch, commit-graph, loose-objects) and filesystem monitor
integration.
...
view support. I chose it for its clean visual presentation.
...
[[https://github.com/jesseduffield/lazygit][Lazygit]] is a terminal UI for git. I chose a TUI over raw git commands for interactive
operations like staging individual hunks, interactive rebase, and conflict resolution,
where the visual feedback keeps me from mis-staging things. I used gitui before, but
switched to lazygit for its broader feature set — gitui's performance advantage did not
materialize in practice, and it lacked several operations my workflow needs.

Performance degrades noticeably on large repositories like nixpkgs, but the ergonomics
and active development are worth that tradeoff to me.
...
I prefer Firefox derivatives over Chromium-based browsers because Firefox's extension
ecosystem and =about:config= give deeper control over browser behavior, and the Nix
ecosystem has mature tooling for managing Firefox profiles declaratively via home-manager.

I chose Zen over vanilla Firefox for its better UI while keeping full Firefox
compatibility — extensions, search engines, and profile settings work identically.
