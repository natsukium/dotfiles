...
Declares virtual machines managed through libvirt using [[https://github.com/AshleyYakeley/NixVirt][NixVirt]], allowing domain
definitions to be expressed declaratively alongside the rest of the system
configuration.

** Host

Configures libvirt, a software TPM emulator, a default NAT network, and VM
viewers. Guests contribute their own static DHCP leases via =dhcpHosts=.

#+begin_src nix :tangle features/virtual-machine/libvirt.nix :exports none
# This file is auto-generated from configuration.org.
# Do not edit directly.
#+end_src

#+begin_src nix :tangle features/virtual-machine/libvirt.nix
{ inputs, ... }:
{
...
Manages a Windows golden image and an interactive guest backed by a persistent
overlay. [[*Windows guest][The Forgejo runner]] shares this same base image.

A logon scheduled task executes =D:\bootstrap.ps1= on the guest's state disk if
present, decoupling guest-specific startup logic from the shared base image.

#+begin_src nix :tangle features/virtual-machine/windows/default.nix :exports none
# This file is auto-generated from configuration.org.
# Do not edit directly.
#+end_src

#+begin_src nix :tangle features/virtual-machine/windows/default.nix
{ inputs, ... }:
{
...
Resource and disk configuration for the Windows guest. =baseImage= and
=payloadIso= are exposed as read-only options so secondary guests can reference
them without coupling to internal directory layout.

#+NAME: windows-vm-options
#+begin_src nix
...
Defines disk paths for the base image, the system overlay (C:), and the
persistent state disk (D:). Unlike ephemeral runner guests, this interactive
guest preserves its overlay across reboots.

#+NAME: windows-vm-disks
#+begin_src nix
...
Builds an ISO carrying provisioning scripts and the SPICE guest agent. The agent
version is pinned in =spice-vdagent.nix= rather than inline so Renovate updates
are not overwritten by tangling.

#+NAME: windows-vm-payload
#+begin_src nix
...
Generates a partitioned FAT disk image containing =autounattend.xml= (for WinPE)
and =unattend.xml= (for post-reboot passes). Windows Setup requires a partitioned
disk to read unattended answers and ignores ISO media. Target disk selection is
omitted to prevent unattended installation from overwriting the wrong drive.

#+NAME: windows-vm-answer-image
#+begin_src nix
answerImage =
...
Extends NixVirt's Windows template for both installer and normal modes.
Reusing a single domain preserves NVRAM variables written during installation,
ensuring the Windows boot loader entry remains intact when booting the overlay.

#+NAME: windows-vm-domain
#+begin_src nix
...
The domain is defined but not started automatically on host activation, as it
is intended for on-demand interactive use.

#+NAME: windows-vm-libvirt
#+begin_src nix
...
Initializes empty qcow2 images for the base image, state disk, and system
overlay on first boot. When =installer= is enabled, synchronizes the installer
ISO into the state directory.

#+NAME: windows-vm-images
#+begin_src nix
...
=provision.ps1= configures the guest environment: virtio drivers, QEMU guest
agent, Git, Node.js, MSVC tools, and the nightly Rust toolchain. Software is
installed via direct downloads because winget is unavailable in unattended
sessions.

Toolchain caches are directed to D: so they persist across overlay resets.
Desktop animations and transparency are disabled to reduce CPU overhead under
the unaccelerated virtio-gpu driver. All steps are idempotent.

#+begin_src powershell :tangle features/virtual-machine/windows/provision.ps1
...
=bootstrap.ps1= runs at user logon to partition and format the state disk as
=D:= on first boot and remount optical drives. It then invokes =D:\bootstrap.ps1=
if present, allowing guests to run custom startup tasks.

#+begin_src powershell :tangle features/virtual-machine/windows/bootstrap.ps1
<#
...
=share.ps1= installs WinFsp and configures the =VirtioFsSvc= service to mount
host directories shared via virtiofs at a designated drive letter. This runs
once on the interactive guest rather than the shared golden image.

#+begin_src powershell :tangle features/virtual-machine/windows/share.ps1
#Requires -RunAsAdministrator
<#
...
To install the golden image, set =installerIso= to an official Windows
installation ISO, enable =installer = true=, and boot the guest.

#+begin_src sh :tangle no
sudo nixos-rebuild switch --flake .
virsh -c qemu:///system start windows
virt-viewer -c qemu:///system windows
#+end_src

Send a key press to satisfy the initial CD-ROM boot prompt. Setup will
automatically format the disk according to the answer file and execute
=provision.ps1= on first logon; only the target disk needs to be selected
manually.

#+begin_src sh :tangle no
virsh -c qemu:///system send-key windows --holdtime 150 KEY_SPACE
virsh -c qemu:///system screenshot windows shot.ppm
#+end_src

Once installation finishes, disable =installer= and rebuild. The domain will
then boot from its overlay disk. Future updates to the golden image can be made
by enabling =installer= again and re-running =provision.ps1=.
...
=hostPackages= adds =nix= and the niks3 client. A host label names the Nix
system its jobs run on (=x86_64-linux=, =aarch64-darwin=, =x86_64-windows=), so
a workflow's =runs-on= states the platform it needs.
...
Declares a Forgejo Actions runner hosted on a Windows guest, reusing the golden
image from [[*Windows][the Windows VM module]].

The guest boots from an ephemeral overlay recreated on each VM start via a
libvirt hook. A persistent secondary disk (D:) preserves the runner
credentials, toolchains, and cargo caches across resets.
...
{ inputs, ... }:
{
  flake.modules.nixos.forgejo-runner-windows =
    ...
    let
      ...
      <<forgejo-windows-disks>>

      <<forgejo-windows-payload>>

      <<forgejo-windows-domain>>
    in
    {
      options.my.services.forgejo-runner.windows = {
        <<forgejo-windows-options>>
      };
      ...
      config = lib.mkIf cfg.enable {
        ...
        <<forgejo-windows-libvirt>>

        <<forgejo-windows-images>>

        <<forgejo-windows-seed>>

        <<forgejo-windows-reset>>
      };
    };
}
#+end_src
...
Runner identity and token settings are inherited from the parent Forgejo runner
configuration. Resource allocations default to half of host threads and a
quarter of RAM to avoid starving other host services during builds.

#+NAME: forgejo-windows-options
#+begin_src nix
...
Configures guest disk paths and MAC address. The state disk persists the runner
registration and toolchain caches across resets. NVRAM variables are cloned from
the base Windows guest to retain EFI boot entries.

#+NAME: forgejo-windows-disks
#+begin_src nix
...
Builds an ISO containing the Windows runner binary and setup scripts. The runner
is cross-compiled with Go rather than =pkgsCross.mingwW64=, whose build hooks
fail on mingw dependencies.

Copying the binary to =D:\runner= on each boot ensures that updates pinned in
the flake take effect even though D: survives VM resets.

#+NAME: forgejo-windows-payload
#+begin_src nix
...
Extends the Windows NixVirt template with the runner, payload, and seed ISOs,
plus the persistent state disk.

#+NAME: forgejo-windows-domain
#+begin_src nix
template = nixvirt.domain.templates.windows {
  name = "windows-ci";
...
Started automatically on activation so the runner remains available.

#+NAME: forgejo-windows-libvirt
#+begin_src nix
virtualisation.libvirt.connections."qemu:///system".domains = [
  {
...
Creates the persistent state disk if missing and seeds NVRAM firmware variables
from the base Windows guest.

#+NAME: forgejo-windows-images
#+begin_src nix
...
Generates a root-only ISO containing the registration token on activation,
keeping credentials out of the Nix store and guest command history.

#+NAME: forgejo-windows-seed
#+begin_src nix
...
A libvirt QEMU hook recreates the system overlay before the VM starts, ensuring
each run begins from a clean state. Guest-initiated reboots retain the overlay;
a domain stop and start via libvirt is required to reset it.

#+NAME: forgejo-windows-reset
#+begin_src nix
...
Initializes a new state disk by running =provision.ps1= to install toolchains,
followed by =register.ps1= to register the runner using the token from the
[[*Seed disc][seed disc]].
...
powershell -ExecutionPolicy Bypass -File "$runner\register.ps1"
#+end_src

#+begin_src powershell :tangle features/forgejo-runner/windows/register.ps1
#Requires -RunAsAdministrator
<#
...
Leaves =D:\bootstrap.ps1=, which the logon task executes on each boot to run the
runner daemon.

#+begin_src powershell :tangle features/forgejo-runner/windows/bootstrap.ps1
<#
