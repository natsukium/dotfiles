...
=auto-optimise-store= used to be Linux-only. The optimiser picked its temporary link name
with an unguarded ~rand()~, which on macOS collided between concurrent builders and failed
them with =error: cannot link '/nix/store/.tmp-link' to '/nix/store/.links/...': File
exists= ([[https://github.com/NixOS/nix/issues/7273][NixOS/nix#7273]]). Nix 2.34 draws that name from =makeTempPath= instead, so both
mechanisms now run everywhere.
...
Each builder agent runs its builds through the local Nix daemon. On a host that
also sets =nix.buildMachines=, that daemon hands the step straight back out over
ssh. Hydra then credits the build to the machine that only relayed it, and both
agents go on counting the slot as free. The legacy queue runner avoided this by
passing =nix-store --serve --builders ''= for localhost builds
([[https://github.com/NixOS/hydra/pull/749][NixOS/hydra#749]]). The Rust rewrite dropped that guard, so I patch it back in.
Nix lets any client clear =builders=, even an untrusted one, so the agent only
has to send the setting.

The patch goes on the workspace derivation, not on the source tree. Patching the
tree first would mean reading the workspace expression out of a built copy, and
that import from derivation fails under =nix flake check --no-build=, which
refuses to realise anything. Upstream's per-crate wrapper copies its binary out
of the unpatched workspace, so the feature lifts =hydra-builder= out of the
patched one itself.

#+begin_src nix :tangle features/hydra/default.nix :exports none
# This file is auto-generated from configuration.org.
# Do not edit directly.
#+end_src

#+begin_src nix :tangle features/hydra/default.nix
{ inputs, ... }:
let
...
Fish completes =launchctl= subcommands but not service targets like
=gui/<uid>/...=. This replaces the built-in completions with dynamic candidates
parsed from =launchctl print= and =launchctl help=, showing each service's
running state alongside its target.

Targets are fully qualified so fish's substring matching can complete a
service without knowing its domain upfront. Subcommands and flags are parsed
on demand from =launchctl help= to match the installed macOS version.

#+NAME: fish-launchctl-completion
#+begin_src nix :noweb no
...
#+begin_src fish :tangle features/shell/fish/completions/launchctl.fish :exports none
# This file is auto-generated from configuration.org.
# Do not edit directly.
#+end_src

#+begin_src fish :tangle features/shell/fish/completions/launchctl.fish
...
A fresh =ghq get= clone is invisible to =z= until I cd into it once. Piping the path
that =ghq get= prints on stdout since 1.10.0 into =zoxide add= registers it right away.

I use an abbreviation rather than a function wrapping =ghq= so the pipe stays visible
and I can delete it when I need =ghq get --look=, whose nested shell the pipe would
swallow.

=xargs -I{}= keeps a path with spaces whole and runs nothing when a failed clone leaves
stdout empty.
...
I am moving my data analysis work to [[https://jj-vcs.github.io/jj/][Jujutsu]], where the history has a shape git
does not expect. Analysis is a long run of small experiments and most of them are
abandoned, so git records only the states I remembered to commit. jj snapshots
the working copy on every command, which means the state I had twenty minutes
ago is still in the operation log whether or not I judged it worth keeping at
the time. Rearranging the survivors into a sequence someone can read is then
=jj squash= and =jj rebase=, instead of an interactive rebase I would have had
to plan for while still exploring.

jj keeps its commits in a git repository and colocates with =.git= by default,
so the git tooling in the [[*Git][Git]] section keeps working inside a jj repo. The two are
registered as separate features because jj is not a replacement here: git
remains what I use everywhere else.

#+begin_src nix :tangle features/jujutsu.nix :mkdirp yes :exports none
# This file is auto-generated from configuration.org.
# Do not edit directly.
#+end_src

#+begin_src nix :tangle features/jujutsu.nix :mkdirp yes
{ ... }:
{
...
*** Shared Settings With Git

jj reads none of git's configuration, so identity, signing, and editor would
have to be written twice. Everything that is literally the same value is read
back out of the [[*Git][Git]] feature instead, which means a key rotation or a change of
address is one edit rather than two that can fall out of step.

That makes jujutsu depend on git being enabled. Without the assertion the
failure would be a missing-attribute error pointing into the module system,
which says nothing about the actual cause.

#+NAME: jj-git-assertion
#+begin_src nix
assertions = [
  {
...
The key is git's, and so is the decision to sign at all: =signByDefault= becomes
jj's =own=, which signs the commits I author. Mapping git's =format= onto jj's
=backend= covers the case where I move off ssh keys, since the two spell the
same three backends differently.

jj rewrites the working-copy commit on every snapshot, which calls the signing
program far more often than git's per-commit signing does. I measured the
difference at a few milliseconds per command, below the cost of the snapshot
itself, so the frequency is not a reason to weaken this to =keep=.

Signature status shows as unknown in =jj log= because I keep no allowed-signers
file. That only affects local verification; GitHub and Forgejo check the
signature against the key registered on my account.
...
A bare =jj= already falls back to =jj log=, but prints a hint naming this
setting on every run until it is set. Setting it buys nothing except silence,
which is reason enough.

The editor is git's, set there because [[*Editor][some contexts do not inherit the session
environment]]; jj falls back the same way and deserves the same answer.

difftastic replaces the built-in colour-words diff. jj invokes a diff formatter
on the two file versions directly, which is the interface difftastic wants, so
no wrapper is needed. delta stays git-only because it reads a unified diff
rather than a pair of files.
...
Automatic snapshotting is what makes jj worth using for analysis, and it is also
the thing that would put a parquet file into the store the moment I dropped one
next to the notebook. jj has no equivalent of git-lfs, so the store would simply
grow. =max-new-file-size= is the guard: a new file larger than the limit is not
tracked at all.

The value below is jj's own default, written out because it carries the rule
that data lives outside the repository. Without that rule the automatic snapshot
turns from the reason I chose jj into the reason I would have to abandon it.

One catch is worth knowing before it bites. jj 0.44 skips an oversized file in
silence: nothing is printed, and =jj status= does not list the file as
untracked, so a file I assumed was committed can turn out not to be. When that
happens the fix is to move the data out of the tree, not to raise the limit.
...
created at service start instead. Stars and forks are off for the same reason:
both are social features, and with no second account they leave nothing but a
button and a counter that never move.

New repositories start out with the two settings I would otherwise pick by hand:
squash merges, and rebase when a pull request branch needs updating. Deleting the
branch after a merge is the third habit, but Forgejo has no instance-level default
for it, so I still tick that box per repository.
...
Forgejo authors the commit itself when I merge a pull request or edit a file in
the web UI, so those commits carry no signature. An instance key signs them, in
the same SSH format as [[*Commit Signing][my own commits]].

#+NAME: forgejo-signing
#+begin_src nix
...
A host enables a wrapper and supplies its own registration token. The
[[*Windows guest][Windows guest]] hangs off the same directory, and is imported by hand because a
directory with a =default.nix= is one module.
...
Software that has to keep working on Windows needs a Windows machine to build
and test it on, so one runner is a Windows guest. This feature declares it as a
libvirt domain through [[https://github.com/AshleyYakeley/NixVirt][NixVirt]] and registers it under a =windows= label. A host
turns it on with =my.services.windows-ci= and hands over the registration token
it already keeps for its Linux runner.

The guest boots on a throwaway overlay of a golden image, and a libvirt hook
recreates that overlay every time qemu starts. So nothing a job leaves behind
outlives the next start of the guest. Only the second disk survives, and it
holds nothing I could not rebuild: the runner credential, the rustup toolchains,
and the cargo registry cache.

Windows cannot be built from this flake, so the golden image is installed once
from an answer file. =installer= switches the domain over for that: the guest
writes into the base image directly, the installation ISO is attached, and the
reset hook is withheld. [[*Installing Windows][Installing Windows]] below is that one-time procedure.

#+begin_src nix :tangle features/forgejo-runner/windows/default.nix :exports none
# This file is auto-generated from configuration.org.
# Do not edit directly.
#+end_src

#+begin_src nix :tangle features/forgejo-runner/windows/default.nix
{ inputs, ... }:
{
...
The host decides what it can afford and where its secrets are, and nothing
else. Sizing is the interesting one: half the host threads and a quarter of its
RAM leaves the desktop, Hydra, and the Linux runner working while a Windows job
compiles. The runner identity carries my defaults the way the Forgejo runner
wrappers do, so a host that just wants a Windows runner sets =enable= and
=tokenFile=.

#+NAME: windows-ci-options
#+begin_src nix
...
The state disk is the one that survives a reset. It carries the runner
credential, because re-registering on every boot would leave a dead runner in
Forgejo each time, and the rustup and cargo caches beside it.

#+NAME: windows-ci-disks
#+begin_src nix
...
Windows owns its disk, but the flake still owns what runs on it. Handing the
guest a disc is the one way in that needs nothing installed on either side, so
anything the guest cannot fetch for itself rides in this way: the runner
binary, the provisioning script, and a SPICE agent.

Forgejo releases runner binaries for Linux only, so the Windows one is built
here by pointing Go at another target. Dropping the git wrapper and the version
check is all else it takes. =pkgsCross.mingwW64= is the other route and costs
more than it returns: the package's =meta.platforms= rules Windows out, and the
wrapper hook it still pulls in wants a mingw bash and gawk that do not build.

The SPICE agent is fetched rather than built, and it stays on the disc because
provisioning the console means pasting into it. Its version is pinned in
=spice-vdagent.nix= rather than here: Renovate rewrites the pin in the tangled
file, which the next tangle of this document would undo.

=provision.ps1= is documented with [[*Provisioning][the guest's own setup]] below, and its runner
identity is substituted in here rather than written into the script.

#+NAME: windows-ci-payload
#+begin_src nix
...
Setup would not read the answer file off a disc, and it cannot go on the
install media either: that ISO keeps its real tree in UDF, which xorriso cannot
write, so rewriting it leaves an image holding nothing but a README. A disk it
does read. The partition table matters as much as the contents, because a bare
filesystem with no table is a disk Windows never mounts.

The same answers go on twice under both names setup looks for. It reads
=autounattend.xml= while WinPE is up and =unattend.xml= for every pass after the
reboot, and the passes that matter here are the later ones.

Two things about the file itself have to be right at once, and while either was
wrong setup just asked every question itself, with no error anywhere. It is
validated against a schema whose sequences are alphabetical, so =AutoLogon=
before =OOBE=, =Password= before =Name=, and =CommandLine= before =Order=. A
file that fails validation is discarded in silence. And the driver paths live in
=Microsoft-Windows-PnpCustomizationsWinPE=, not in =Microsoft-Windows-Setup=
where I first wrote them, which is enough on its own to invalidate the file.

The file names no disk to install onto, so setup asks. It can only name a disk
by number, and this guest's two disks came up in an order I could not predict,
so a wrong guess would wipe the state disk rather than the base image.

#+NAME: windows-ci-answer-image
#+begin_src nix
answerImage =
...
NixVirt's Windows template gives the machine type, the firmware, and the SPICE
devices. What it does not give is anything about this guest in particular, so
the domain is the template with a handful of devices replaced.

#+NAME: windows-ci-domain
#+begin_src nix
...
The guest sits on its own NAT network with a fixed lease, so the host always
knows where to find it.

#+NAME: windows-ci-libvirt
#+begin_src nix
...
Both qcow2 images are created empty on first boot, and neither is ever
recreated by this unit. The base image is what an install writes into, so
removing it is the only way to start over.

#+NAME: windows-ci-images
#+begin_src nix
...
The token is the one thing the guest cannot be handed at build time. Building
the disc at activation keeps it out of the Nix store, and stripping the
=TOKEN== prefix keeps the guest from having to parse an environment file. qemu
runs as root here, so the disc can stay readable only by root. A token typed at
the console instead would sit in the shell history, and in the golden image with
it.

#+NAME: windows-ci-seed
#+begin_src nix
...
The system disk is an overlay over the base image, and a libvirt hook replaces
it every time qemu starts. A hook rather than a boot-time unit, because the
overlay cannot be swapped out while qemu holds it open. The same limit means only
a fresh qemu process gets a fresh disk: a reboot from inside Windows keeps
whatever the last job left behind, and only a stop and start through libvirt
clears it.

Host shutdown stops the guest rather than saving it. A memory image restored
onto a system disk that no longer exists is a guest that never comes back, and
libvirt-guests starts a stopped guest again on the next boot anyway.

#+NAME: windows-ci-reset
#+begin_src nix
...
Setup injects only the two drivers it cannot boot without, so =provision.ps1=
runs the virtio guest tools from the virtio disc first, and then the guest agent
package beside it, which the tools leave out. The agent is what makes the guest
reachable at all: with it, =virsh qemu-agent-command= runs PowerShell in there,
and the rest of provisioning needs no console.

That disc carries no Joliet names, so every hyphen in a filename arrives as an
underscore, and the script matches its files with wildcards rather than by name.

Everything else a job needs has to sit in the golden image, because C: returns
to that image on every start. Actions cannot stand in for this. Both
=dtolnay/rust-toolchain= and =actions-rust-lang/setup-rust-toolchain= assume
rustup is already installed, and pulling the MSVC toolchain per job would cost
gigabytes and ten minutes of every run.

None of it goes through winget. winget is an AppX package, so whether it exists
at all depends on the account running it and on the Store having updated App
Installer, and the first run of this on a fresh install answered with "winget is
not recognized". Vendor installers have neither problem.

Native tools are called through a wrapper that drops =ErrorActionPreference=
for the call. Otherwise a single line on stderr ends the script, and these tools
talk on stderr for reasons that have nothing to do with failure: rustup
reporting =info: using existing install= stopped provisioning halfway through,
twice, and left no error behind to explain it.

So the whole of it lives in =provision.ps1=: the state disk, the machine-scoped
=RUSTUP_HOME= and =CARGO_HOME= that keep the caches on D:, git, Node.js, rustup
with the nightly toolchain, the MSVC build tools, cargo-nextest, and the runner
itself. Node is not for any project: the runner executes JS actions —
=actions/checkout= before anything else — with a node from the host PATH, and
its first job without one died at step one with "Cannot find: node in PATH". A
task registered at logon copies the runner across to =D:\runner= and runs the
daemon as the =ci= user in its own interactive session — session 0, where a
SYSTEM task runs, has no desktop, and a job that opens a window and a wgpu
swapchain fails there with "Invalid surface" before its first frame, so the
account autologs on permanently and the runner rides that session. Copying on
every boot keeps the version the flake pins as the
version that runs, even though D: outlives the resets that would otherwise carry
the update. Registration writes =.runner= beside the working directory, so the
daemon runs from D: for the credential to survive a reset.

The answer file runs it at first logon, so a fresh install needs no command from
me. Every step checks for its own result first, so re-running it after a change
is safe. That is one line.

#+begin_src powershell :tangle no
$payload = (Get-Volume -FileSystemLabel WINCI).DriveLetter + ":"
powershell -ExecutionPolicy Bypass -File "$payload\provision.ps1"
#+end_src

No token appears in that line. The script reads it off the [[*Seed disc][seed disc]].

**** Installing Windows

Microsoft hands out a download link per session rather than a stable URL.
quickget knows how to ask for one, but Microsoft refused the automated request
from my home address. So the ISO comes off the download page in a browser
instead. Nothing reads this guest's UI, so I take the English image rather than
the Japanese one.

Point =installerIso= at the result, turn =installer= on, and start the guest.

#+begin_src sh :tangle no
sudo nixos-rebuild switch --flake .
virsh -c qemu:///system start windows-ci
virt-viewer -c qemu:///system windows-ci
#+end_src

A NixOS rebuild redefines the domain, and NixVirt stops the guest to apply the
new definition. So do not rebuild while setup is running.

Setup reads the answer file at the start of each pass. It creates the local =ci=
administrator, keeps OOBE from ever asking for a Microsoft account, and runs
=provision.ps1= at first logon, so the install ends with a registered runner and
no keystrokes of mine in it. It still asks which disk to install onto, because
[[*The answer disk][the answer file]] names none. Picking the 80 GiB one by hand costs three key
presses.

Once a pass has begun it is too late to fix its answers. No amount of dropping
files into =C:\Windows\Panther= or pointing =HKLM\System\Setup!UnattendFile= at
one changes anything, so a bad file means installing again rather than repairing
in place.

The interactive route is where I started, and it has a dead end. Windows 11
25H2 wants a Microsoft account before it will finish OOBE, and this guest has
no network to sign in with. =bypassnro.cmd= is gone from this build and
=ms-cxh:localonly= opens nothing. The =BypassNRO= registry flag does get past
the account wall, but by then I had also tried =sysprep /audit=, which left the
image believing setup was still running. The task scheduler in that state
queues every task in the system and launches none of them, including
Microsoft's own, so the runner could never start at boot. Repairing the
registry markers by hand did not bring it back.

The firmware asks to press a key to boot from CD. Nothing else on the guest is
bootable at that point, so it waits rather than falling through, and
=virsh send-key= answers it. The rest of setup can be driven the same way, and
=virsh screenshot= shows where it is.

#+begin_src sh :tangle no
virsh -c qemu:///system send-key windows-ci --holdtime 150 KEY_SPACE
virsh -c qemu:///system screenshot windows-ci shot.ppm
#+end_src

Turning =installer= back off and rebuilding gives the domain its overlay and its
reset hook, and activation starts it. The copied ISO under the state directory
...
Declaring the container pins its ID so routing rules can address it by
=firefox-container-1=. =containersForce= allows home-manager to write
=containers.json=, which Firefox overwrites on launch.
...
Mozilla's Multi-Account Containers only matches on domain. [[https://github.com/kintesh/containerise][containerise]] matches
paths, routing only =attmcojp= URLs into the work container.

Rules only route inbound: routing non-work paths back out breaks sign-in, as
container switches reopen tabs and drop POST bodies (such as SAML assertions
and two-factor forms).

#+NAME: firefox-containerise-rules
#+begin_src nix
...
The built-in =switch_session= binding only steps one session at a time.
=felis-switch= (=Ctrl+Shift+p=, via the =run= action) instead pipes =felis
sessions list= through fzf, previewing each session's grid in colour, and
switches to the pick. It and its two siblings read the list as =--format json=:
felis promises that shape and explicitly reserves the right to move the human
table's columns, which is what broke them the last time the CLI changed.
...
rebuilds the grid itself and drifts on a wide character or soft wrap. The pick
opens rather than landing back at the prompt: felis now exports the originating
session id, so typing it back is possible, but opening is what I reach for.
