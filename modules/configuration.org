...
The performance settings themselves are only applied once a repository is actually
registered (=scalarCfg.enable=, which the block above turns on), so a host that
never touches a large repository never pays for tracking it.
