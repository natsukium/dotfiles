    ...
    in
    {
      ...
      config = lib.mkIf (enabledInstances != { }) {
        ...
        system.activationScripts.postActivation.text = lib.concatMapStringsSep "\n" (instance: ''
          if [ -d ${lib.escapeShellArg instance.stateDir} ]; then
...
in
{
  # Internal registry (_class-checked): hosts enable my.services.forgejo-runner.
  flake.modules.darwin.forgejo-runner = darwinWrapper;
  flake.modules.nixos.forgejo-runner = nixosWrapper;

  # Public API: the neutral Darwin mechanism, safe to promise (no personal opinion).
  flake.darwinModules.forgejo-runner = darwinModule;
}
