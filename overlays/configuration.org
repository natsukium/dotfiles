...
#+begin_src nix :tangle default.nix :exports none
...
Firefox takes onnxruntime as a build input on Linux since 141, and Calibre does the same for
its Python environment. A host-wide =cudaSupport= would drag both applications off
cache.nixos.org and rebuild their CUDA-enabled onnxruntime dependencies from source.
...
nixpkgs branch. Overriding an attribute such as =doCheck=, or applying a patch to the
existing package, is enough. Remove these overrides once the issues are fixed upstream.
...
The 0.6.27 bump left the console script pointing at =calibreweb.__main__:main=, but
nixpkgs moves upstream's =cps.py= to =calibreweb/__init__.py=, so no =__main__= module
is ever installed.
...
manyara serves Grafana over MCP behind caddy and to the hermes guest, so both
clients arrive with a =Host= that is not loopback. In 1.2.0 the =--allowed-hosts=
flag only reached mcp-grafana's own middleware: the mcp-go server underneath kept
its own loopback-only check and answered =invalid Host header=. That check runs
after caller authentication, so unauthenticated probes returned 401 and looked
healthy, while the one client that actually held a token was the only one to see
the 403. Upstream wired the flag through to the inner server in 1.4.0.

One dashboard test talks to a local test server and fails under the build sandbox
with a truncated response, so it is skipped rather than turning off the whole
suite.

Drop this once nixpkgs ships 1.4.0 or newer.
