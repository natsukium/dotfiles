...
I organize overlays into four categories by purpose and expected lifetime:
...
Unlike the =stable= overlay, these don't fetch packages from a different nixpkgs
branch; they only override specific attributes (like =doCheck=) on the existing
packages. Remove these overrides once the issues are fixed upstream.
...
Python packages in nixpkgs form an interconnected dependency graph. Going through
=packageOverrides= makes every dependent package see the modified version. A direct
overlay override (e.g., =python313Packages.foo = ...=) would only change top-level
access, leaving internal dependencies on the original broken version.
...
The test suite causes a segmentation fault during execution; I have not tracked
down the root cause.

This package is only pulled in as a transitive dependency, and it has worked
correctly in actual use, so disabling the test suite is a workaround I can live with.
...
encoding support, which fixes the rendering issue. I wrap the real binary with
=writeShellApplication= so that variable is set before it runs.
