...
Firefox takes onnxruntime as a build input on Linux since 141, so a host-wide =cudaSupport=
drags the browser off cache.nixos.org and rebuilds it from source.
