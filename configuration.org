...
Lightweight per-VM isolation. Used on manyara to sandbox third-party agent code (hermes-agent) so
crashes, runaway tool invocations, or compromise of the agent runtime cannot reach the host's
service stack (matrix homeserver, syncthing, calibre data, etc.). A microvm is preferred over a
plain systemd-nspawn container because the qemu/firecracker boundary blocks kernel-level escapes
that container namespacing alone does not.
...
Provides the upstream NixOS module (=nixosModules.default=) and packaged Python build of
hermes-agent, used by the matrix-facing agent on manyara. The flake is consumed instead of
shipping our own packaging because hermes pulls in a deep Python/uv2nix stack that would be
costly to replicate and re-vendor on every release.
