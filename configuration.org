...
https://github.com/vicinaehq/extensions

#+BEGIN_QUOTE
Extensions for the Vicinae launcher
#+END_QUOTE

Prebuilt Vicinae extension packages, one per directory in the repo. Referenced by
the launcher module so extensions like the Nix one are installed declaratively
instead of through Vicinae's GUI store.
...
Each per-system concern lives in its own =modules/flake/per-system/*.nix= file,
...
#+begin_src nix :tangle modules/flake/per-system/pkgs.nix
{ self, inputs, ... }:
{
  perSystem =
    { system, ... }:
...
#+begin_src nix :tangle modules/flake/per-system/checks.nix
{ inputs, ... }:
{
  perSystem =
    { pkgs, ... }:
...
#+begin_src nix :tangle modules/flake/per-system/packages.nix
{ ... }:
{
  perSystem =
    {
...
#+begin_src nix :tangle modules/flake/per-system/dev-shell.nix
{ ... }:
{
  perSystem =
    {
...
#+begin_src nix :tangle modules/flake/per-system/pre-commit.nix
{ inputs, ... }:
{
...
#+begin_src nix :tangle modules/flake/per-system/treefmt.nix
{ inputs, ... }:
{
...
#+begin_src nix :tangle modules/flake/per-system/mcp-servers.nix
{ inputs, ... }:
{
