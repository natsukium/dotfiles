...
Hydra's Rust queue runner and builder are developed in the Hydra repository, while
nixpkgs only exposes the legacy service module.
...
#+BEGIN_QUOTE
Declarative definitions for libvirt
#+END_QUOTE

NixVirt renders libvirt domain XML from Nix and reconciles the definition on
activation, so a guest whose disk image this flake cannot build is still
described by it. microvm.nix covers the other case, where the guest is a NixOS
system built here.

I take the flake from FlakeHub rather than GitHub because upstream warns that
the master branch is frequently broken. The wildcard resolves to the latest
release.
...
Binary cache for CUDA-related packages. These were distributed under the nix-community
namespace before, and since November 2025 come from the CUDA team's own infrastructure.
Build status can be monitored at https://hydra.nixos-cuda.org/project/nixos-cuda

The nixos-cuda team describes this cache as being for development purposes only.
...
kilimanjaro is the host with the cores to spare, so it runs the
[[file:modules/configuration.org::*Windows guest][Windows guest]]. It hands the guest the token the Linux runner here already
uses, and the installation ISO off the NTFS partition it shares with the dual
boot. [[file:modules/configuration.org::*Installing Windows][Installing Windows]] is the one-time procedure. I have not run it end to end
yet.
...
Entry also exports the bearer token for the Grafana MCP server, but only while
=rbw= is already unlocked. Fetching it unconditionally would put a pinentry
prompt in front of every =direnv= entry, which costs more than the occasional
session that starts without the token.
...
The Grafana server runs on manyara rather than as a local subprocess, so the
Grafana credentials stay on the host that already holds them and a client needs
only a URL. This shell talks to the writable instance; the read-only one belongs
to hermes, which runs unattended.

The generated config lands in the world-readable store, so the =Authorization=
header keeps a =${MCP_GRAFANA_TOKEN}= reference that the client expands from the
environment, and the development shell exports the value from Bitwarden.
...
[type: org] modules/features/editor/emacs/init.org $lang:modules/features/emacs/init.$lang.org opt:"-k 0"
[type: org] modules/features/editor/emacs/early-init.org $lang:modules/features/emacs/early-init.$lang.org opt:"-k 0"
