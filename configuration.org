...
https://github.com/natsukium/nix-mac-app-identity

#+BEGIN_QUOTE
Preserve macOS TCC permissions across Nix rebuilds with stable code signing identities
#+END_QUOTE

Nixpkgs apps are ad-hoc signed, so macOS tracks TCC permissions by binary hash and
revokes them on every rebuild. Re-signs bundles and wraps binaries with stable
code signing identities so granted permissions persist across updates.

#+NAME: nix-mac-app-identity
#+begin_src nix
nix-mac-app-identity = {
...
#+begin_src emacs-lisp :tangle scripts/export-agents-md.el :mkdirp yes
(require 'ox-md)
...
and syncs =AGENTS.md= from the literate source.
