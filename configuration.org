...
Each machine is a directory under =hosts/<platform>/<name>/=. I add or remove a
host by adding or removing its directory: =modules/flake/hosts.nix= reads the
tree and builds a configuration for every entry, so there is no list to keep in
sync here. The platform segment (=nixos=, =darwin=, =android=) selects the
constructor, and the architecture lives in =nixpkgs.hostPlatform= inside each
host body.

The subsections below keep the per-host notes (hardware, BIOS, setup steps).
